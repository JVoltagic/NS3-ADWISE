...
OFSwitch13Port::NotifyConstructionCompleted ()
{
  ...
  Ptr<WaveNetDevice> WaveDev = m_netDev->GetObject<WaveNetDevice> ();
  ...
  if (csmaDev)
    ...
    {
      WaveDev->SetOpenFlowReceiveCallback (
        MakeCallback (&OFSwitch13Port::Receive, this));
    }
  else
    {
...
Ptr<WaveNetDevice> WaveDev = m_netDev->GetObject<WaveNetDevice> ();
  ...
  uint32_t channel = CCH; 

  seq = uint32_t(seq + 1);
  seqTs.SetSeq (seq);
  packetCopy->AddHeader (seqTs);
  const static uint16_t WSMP_PROT_NUMBER = 0x88DC;
  Mac48Address bssWildcard = Mac48Address::GetBroadcast ();
  const TxInfo txInfo = TxInfo (channel);
  status = WaveDev->SendX  (packetCopy, bssWildcard, WSMP_PROT_NUMBER, txInfo);

    }else{

  // Send the packet over the underlying net device.
  status = m_netDev->SendFrom (packetCopy, header.GetSource (),
